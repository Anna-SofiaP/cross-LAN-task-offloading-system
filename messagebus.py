...
TAG = "[MessageBus]"
TOPIC_HEARTBEAT = "cluster.heartbeat"
...
class MessageBus:
    ...
    def __init__(self, node_id: str, nats_url: str, heartbeat_interval=30):
...
# TODO: make work!
    '''async def send(self, to: str, msg: Message):
        ...
        if self._is_local(to):
            ...
            await self._send_nats(to, msg)

    async def request(self, to: str, msg: Message, timeout: float = 3.0) -> Optional[Message]:
    ...
    # connect() --> connect to NATS, start ZMQ listener
    # _connect_nats() --> connect to NATS, subscribe to subjects
    # _start_zmq_listener() --> bind ROUTER socket, start receive loop
...
#        await self.nc.subscribe(f"nodes.{self.node_id}", cb=self._on_nats_message)
    ...
    # ==================================================================
    # PEER MANAGEMENT
    # _is_local() → check if a node ID corresponds to a local peer
    # _update_peer() → add/update a peer in the registry, call callbacks
    # get_available_peers() → return list of node IDs currently connected to the cluster

    def _is_local(self, node_id: str) -> bool:
        peer = self.peers.get(node_id)
        return peer is not None and peer.get("local", False)
...
# TODO: make work!
    ...
    # ==================================================================
    # NATS transport
    # _send_nats() → publish a message to another node's subject (pub/sub)
...
# TODO: make these work!
    '''async def _send_nats(self, to: str, msg: Message):
        await self.nc.publish(f"nodes.{to}", json.dumps(asdict(msg)).encode())

    async def _request_nats(self, to: str, msg: Message, timeout: float) -> Optional[Message]:
        try:
            ...
            return Message(**json.loads(reply.data))
        except Exception as e:
            print(f"{TAG} NATS request to {to} failed: {e}")
            return None

    async def _on_nats_message(self, raw_msg):
        """Dispatch an incoming NATS direct message to the registered handler."""
        try:
            msg = Message(**json.loads(raw_msg.data))
            result = await self._dispatch(msg)
            if raw_msg.reply and result is not None:
                await self.nc.publish(raw_msg.reply, json.dumps(result).encode())
        except Exception as e:
...
NODE_HEARTBEAT = "cluster.heartbeat"
...
class MessageBus:
    def __init__(self, node_id: str, nats_url: str):
        self.node_id = node_id
        self.nats_url = nats_url
        self.nc = None
        self._handlers = {}
    ...
    async def send(self, to: str, msg: Message):
        if to in self.local_peers:
            await self._send_direct(to, msg)
        else:
            await self._send_nats(to, msg)
'''